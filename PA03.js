var scene, renderer;
	...
	function createMainScene(){
		...
		scene.add(winnerBox);
		winnerBox.addEventListener('collision',function(other_object){
			if (other_object==avatar){
	...
	function updateNPC(){
		...
		if(avatar.position.distanceTo(npc2.position)<500){
			npc2.setLinearVelocity(npc2.getWorldDirection().multiplyScalar(15));
		}
	}
	...
	function updateAvatar(){
		...
		if (controls.fly){
			avatar.setLinearVelocity(new THREE.Vector3(0,controls.speed*5,0));
			avatar.setAngularVelocity(new THREE.Vector3(0,0,0));
		}
		if (controls.left){
